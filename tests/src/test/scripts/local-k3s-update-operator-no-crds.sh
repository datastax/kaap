...
docker inspect k8saap-local-k3s-network  | jq -r '.[0].Containers[].Name' | while read container; do
  ...
  docker exec -t $container sh -c "(ctr image rm docker.io/datastax/k8saap:latest-dev) || (docker image rm -f docker.io/datastax/k8saap:latest-dev) || echo 'not able to remove'"
